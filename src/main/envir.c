...
static inline void InitElemWithBinding(darray_elem *e, SEXP table, SEXP binding){
    e->symbol = TAG(binding);
    e->binding = binding;
...
static dynam_array *NewDynamArray(SEXP table, SEXP symbol, SEXP value){
    ...
    SEXP binding = CONS(value, R_NilValue);
    PROTECT(binding);
    ...
    if (!d) {
	UNPROTECT(1); 
	error("Cannot allocate memory for hash table");
    }


    InitElemWithBinding(&d->elem[0], table, binding);
    d->nelem = 1;
...
static dynam_array *AppendToDynamArray(dynam_array *d, SEXP table, SEXP symbol, SEXP value){
    ...
    InitElemWithBinding(&nd->elem[nd->nelem], table, binding);
    nd->nelem += 1;
...
static dynam_array *NewDynamArrayWithBinding(SEXP table, SEXP binding){
...
static dynam_array *AppendBindingToDynamArray(dynam_array *d, SEXP table, SEXP binding){
...
static void R_EnvHashSetBinding(SEXP binding, SEXP table)
{
    ...
    i = ARRAY_SLOT(a,symbol);
    a->slot[i] = (a->slot[i]) ? 
	AppendBindingToDynamArray(a->slot[i], table, binding) :
	NewDynamArrayWithBinding(table, binding);
...
/*----------------------------------------------------------------------
  ...
  'symbol'. Returns 'R_UnboundValue' if value is not present.
...
/*----------------------------------------------------------------------
  ...
  Note: Creates a temporary LISTSXP with value and symbol, similar
     to Object Tables. JRH
...
/*----------------------------------------------------------------------
  ...
  Returns a new environment with a hash table.
...
/*----------------------------------------------------------------------
  ...
  They have their value set to 'R_UnboundValue'. Returns 0 if symbol
  did not exist, 1 if symbol is found and deleted.
...
static int R_EnvHashDelete(SEXP symbol, SEXP table)
{
    ...
    if (symbol==R_TmpvalSymbol){
	if (a->tmp_binding!=R_NilValue){
	    SETCAR(a->tmp_binding,R_UnboundValue);
	    LOCK_BINDING(a->tmp_binding);
	    R_ReleaseObject(a->tmp_binding);
	    a->tmp_binding = R_NilValue;
	    return 1;
	}
...
static SEXP R_EnvHashFrame(SEXP rho)
{
    SEXP frame, table;
...
void R_EnvUnHashFrame(SEXP rho)
{
    ...
    /* Do some checking */
    if (TYPEOF(rho) != ENVSXP)
	error("first argument ('table') not of type ENVSXP, from R_EnvUnHashFrame");
    table = HASHTAB(rho);
    frame = FRAME(rho);
    if (table == R_NilValue)
	error("HASHTAB is NULL, from R_EnvUnHashFrame");
    if (!IS_ENVHASHTABLE(table))
    ...
    while (found){
	if (frame == R_NilValue) {
	    frame = cursor.elem->binding;
	} else {
	    CDR(frame) = cursor.elem->binding;
	    frame = CDR(frame);
	}
	R_EnvHashCursorNext(&cursor, &found);
    }
...
/*----------------------------------------------------------------------
  ...
  Hash Tables: Only here for R_StringHash. JRH
...
void attribute_hidden unbindVar(SEXP symbol, SEXP rho)
{
	...
	found = R_EnvHashDelete(symbol, HASHTAB(rho));
	/* we have no record here if deletion worked */
	if (found && rho == R_GlobalEnv) R_DirtyImage = 1;
    }
}
...
SEXP findVarInFrame3(SEXP rho, SEXP symbol, Rboolean doGet)
{
	...
	return(R_EnvHashGet(symbol, HASHTAB(rho)));
    }
    return R_UnboundValue;
}
...
static Rboolean existsVarInFrame(SEXP rho, SEXP symbol)
{
	...
	/* Will return 'R_UnboundValue' if not found */
	return R_EnvHashExists(symbol, HASHTAB(rho));
    }
    return FALSE;
}
...
void defineVar(SEXP symbol, SEXP value, SEXP rho)
{
	...
	if (HASHTAB(rho) == R_NilValue) {
	    ...
	    R_EnvHashSet(symbol, HASHTAB(rho), value, FRAME_IS_LOCKED(rho));
	}
    }
}
...
static int RemoveVariable(SEXP name, int hashcode, SEXP env)
{
    ...
    if (IS_HASHED(env)) {
	found = R_EnvHashDelete(name,HASHTAB(env));
	if (found) {
...
static int HashTableSize(SEXP table, int all)
{
	...
	R_EnvHashCursorNext(&cursor, &found);
	while (found){
	    if (NONEMPTY_(cursor.elem->binding))
		count++;
	    R_EnvHashCursorNext(&cursor, &found);
	}
    }
...
#         define DO_HashValues						\
	    SEXP value = CAR(cursor.elem->binding);			\
	    if (TYPEOF(value) == PROMSXP) {				\
		PROTECT(value);						\
		value = eval(value, R_GlobalEnv);			\
		UNPROTECT(1);						\
...
void R_LockEnvironment(SEXP env, Rboolean bindings)
{
	...
	if (HASHTAB(env) && IS_ENVHASHTABLE(HASHTAB(env))) {
	    ...
	    R_EnvHashCursorNext(&cursor, &found);
	    while (found){
		LOCK_BINDING(cursor.elem->binding);
		R_EnvHashCursorNext(&cursor, &found);
	    }
	}
	else {
